...
self-learning memory loop this repo uses to accumulate patterns and corrections over time, so the new project begins building its own project-specific memory from day one rather than starting from a blank slate every session.
