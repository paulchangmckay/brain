...
Before defining tasks, map out which files will be created or modified and what each one is responsible for. For non-trivial features, spawn a `code-architect` subagent first to produce a blueprint (files, component responsibilities, data flows) — use that output to inform the task structure. This is where decomposition decisions get locked in.
