...
- **Submodule commit transfer (worktree → main):** worktrees get independent submodule clones — a commit made in one is invisible to the other. To land it: create a named branch at that commit in the source clone, then from the target clone `git fetch <path-to-source-submodule-clone> <branch-name>` and `git branch -f local-customizations FETCH_HEAD`. A raw SHA is not fetchable via plain local-path fetch.
