# anatomy.md

> Auto-maintained by OpenWolf. Last scanned: 2026-07-20T04:03:43.057Z
> Files: 517 tracked | Anatomy hits: 0 | Misses: 0

## ../Desktop/NHL Stats Project/.claude/worktrees/35-scaffold-react-frontend/

- `README.md` — Project documentation (~851 tok)

## ./

- `.DS_Store` (~3275 tok)
- `.gitignore` — Git ignore rules (~331 tok)
- `.gitleaks.toml` (~143 tok)
- `.gitmodules` (~99 tok)
- `.last-cleanup` (~7 tok)
- `.last-update-result.json` (~46 tok)
- `.shellcheckrc` — Companion enforcement for skills/senior-engineering-partner SKILL.md: (~81 tok)
- `CLAUDE.md` — System Instructions: Thin Harness, Fat Skills Environment (~2975 tok)
- `eslint.config.js` — ESLint flat configuration (~365 tok)
- `history.jsonl` (~40 tok)
- `mcp_servers.json` (~64 tok)
- `mcp-needs-auth-cache.json` (~52 tok)
- `package-lock.json` — npm lock file (~9251 tok)
- `package.json` — Node.js package manifest (~106 tok)
- `settings.json` (~2909 tok)

## .claude/

- `settings.json` (~504 tok)

## .claude/rules/

- `openwolf.md` (~375 tok)

## .claude/worktrees/architecture-diagram-skill/

- `.gitignore` — Git ignore rules (~331 tok)
- `.gitleaks.toml` (~143 tok)
- `.gitmodules` (~99 tok)
- `.shellcheckrc` — Companion enforcement for skills/senior-engineering-partner SKILL.md: (~81 tok)
- `CLAUDE.md` — System Instructions: Thin Harness, Fat Skills Environment (~2975 tok)
- `eslint.config.js` — ESLint flat configuration (~365 tok)
- `history.jsonl` (~40 tok)
- `mcp_servers.json` (~64 tok)
- `package-lock.json` — npm lock file (~9251 tok)
- `package.json` — Node.js package manifest (~106 tok)
- `settings.json` (~2909 tok)

## .claude/worktrees/architecture-diagram-skill/.claude/

- `settings.json` (~504 tok)

## .claude/worktrees/architecture-diagram-skill/.claude/rules/

- `openwolf.md` (~375 tok)

## .claude/worktrees/architecture-diagram-skill/.githooks/

- `pre-commit` — Local pre-commit lint/secret-scan gate — enforcement counterpart to (~455 tok)

## .claude/worktrees/architecture-diagram-skill/.superpowers/sdd/

- `.gitignore` — Git ignore rules (~1 tok)
- `progress.md` — Progress Ledger — architecture-diagram-skill (~661 tok)
- `review-5330aa7..5a09b20.diff` — Review package: 5330aa7bc9923a02acc5fdc4510c237ff277da9c..5a09b20 (~3040 tok)
- `review-5330aa7..620b8de.diff` — Review package: 5330aa7bc9923a02acc5fdc4510c237ff277da9c..HEAD (~3762 tok)
- `review-5330aa7..b9e58e1.diff` — Review package: 5330aa7bc9923a02acc5fdc4510c237ff277da9c..HEAD (~3614 tok)
- `review-620b8de..2b6281a.diff` — Review package: 620b8de7e694017edb19ee25eece1e0252faba9e..2b6281a708b9eb6cb6fd61685a22455125e91168 (~5236 tok)
- `review-c1c6973..5330aa7.diff` — Review package: c1c69739ead2081f09d7349aa81d45c0874629f1..5330aa7 (~671 tok)
- `review-c1c6973..c1c6973.diff` — Review package: c1c69739ead2081f09d7349aa81d45c0874629f1..HEAD (~28 tok)
- `review-c1c6973..f29527a.diff` — Review package: c1c69739ead2081f09d7349aa81d45c0874629f1..f29527a (~12692 tok)
- `review-f29527a..13dade3.diff` — Review package: f29527a3b28c13f5a3e30f2c17664475250c1f18..13dade3 (~5215 tok)
- `task-1-brief.md` — Format: Architecture Diagram (~842 tok)
- `task-1-report.md` — Task 1 Report: Add the Architecture Diagram format to `brand-guide.md` (~655 tok)
- `task-2-brief.md` — Architecture Diagram Skill (~3220 tok)
- `task-2-report.md` — Task 2 Report: Write SKILL.md and copy upstream LICENSE (~2547 tok)
- `task-3-brief.md` — ## Task 3: Write `resources/template.html` (~5237 tok)
- `task-3-report.md` — Task 3 Report: Write `resources/template.html` (~861 tok)
- `task-4-brief.md` — ## Task 4: Generate and visually verify a sample diagram (~4027 tok)
- `task-final-review-fix-report.md` — Final Review Fix Report (~1509 tok)

## .claude/worktrees/architecture-diagram-skill/.understand-anything/

- `.understandignore` — # **/*Test.java (~256 tok)
- `fingerprints.json` (~116279 tok)
- `knowledge-graph.json` (~127512 tok)
- `meta.json` (~45 tok)

## .claude/worktrees/architecture-diagram-skill/.understand-anything/intermediate/

- `scan-result.json` (~18834 tok)

## .claude/worktrees/architecture-diagram-skill/brand/

- `brand-guide.md` — Paul McKay — Personal Brand Guide (~4040 tok)

## .claude/worktrees/architecture-diagram-skill/chrome/

- `chrome-native-host` — Chrome native host wrapper script (~48 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/

- `.gitignore` — Git ignore rules (~4 tok)
- `blume.config.ts` (~68 tok)
- `package-lock.json` — npm lock file (~150416 tok)
- `package.json` — Node.js package manifest (~105 tok)
- `README.md` — Project documentation (~765 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/

- `index.mdx` — ~/.claude Reference (~286 tok)
- `openwolf.md` (~244 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/overview/

- `core-philosophy.md` (~131 tok)
- `infrastructure-layer.md` (~239 tok)
- `knowledge-graph.md` (~197 tok)
- `new-project-bootstrap.md` (~134 tok)
- `personal-knowledge-layer.md` (~202 tok)
- `process-layer.md` (~214 tok)
- `reference-pointers.md` (~121 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/

- `ba.md` — ba (~6228 tok)
- `bloat-audit.md` — Tags (~413 tok)
- `brainstorming.md` — Brainstorming Ideas Into Designs (~3045 tok)
- `brand.md` — Brand Skill — Paul McKay Personal Brand (~1168 tok)
- `claude-infra-reference.md` — Claude Infra Reference (~1577 tok)
- `codebase-design.md` — Codebase Design (~1522 tok)
- `dataviz.md` — DataViz Skill — Executive Data Visualization (~1997 tok)
- `debt-ledger.md` — Scan (~202 tok)
- `divergent-ideation.md` — Divergent Ideation (~732 tok)
- `domain-modeling.md` — Domain Modeling (~822 tok)
- `exec-dashboard.md` — Executive Dashboard Skill (~5490 tok)
- `github-issue-first.md` — GitHub Issue First (~1073 tok)
- `grill-me.md` (~31 tok)
- `grilling.md` (~168 tok)
- `handoff.md` (~241 tok)
- `html-export.md` — HTML Export Skill — Playwright Screenshot & PDF (~725 tok)
- `issue-backlog-cycle.md` — Issue Backlog Cycle (~1729 tok)
- `model-routing.md` — Model Routing (~517 tok)
- `receiving-code-review.md` — Code Review Reception (~1588 tok)
- `requesting-code-review.md` — Requesting Code Review (~731 tok)
- `senior-engineering-partner.md` — ROLE AND CONTEXT (~21885 tok)
- `session-reflect.md` — Session Reflect Skill (~835 tok)
- `systematic-debugging.md` — Systematic Debugging (~2467 tok)
- `teach.md` — Teaching Workspace (~2421 tok)
- `test-driven-development.md` — Test-Driven Development (TDD) (~2473 tok)
- `using-git-worktrees.md` — Using Git Worktrees (~1868 tok)
- `verification-before-completion.md` — Verification Before Completion (~1039 tok)
- `wolf-init.md` — Wolf Init Skill (~505 tok)
- `writing-plans.md` — Writing Plans (~1815 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/brainstorming/

- `spec-document-reviewer-prompt.md` — Spec Document Reviewer Prompt Template (~436 tok)
- `visual-companion.md` — Visual Companion Guide (~3305 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/codebase-design/

- `DEEPENING.md` — Deepening (~638 tok)
- `DESIGN-IT-TWICE.md` — Design It Twice (~673 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/domain-modeling/

- `ADR-FORMAT.md` — ADR Format (~686 tok)
- `CONTEXT-FORMAT.md` — CONTEXT.md Format (~572 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/requesting-code-review/

- `code-reviewer.md` — Code Reviewer Prompt Template (~1301 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/senior-engineering-partner/

- `README.md` — Project documentation (~4517 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/senior-engineering-partner/.github/

- `pull_request_template.md` — What changed (~318 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/senior-engineering-partner/evals/

- `README.md` — Project documentation (~828 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/senior-engineering-partner/references/

- `audit-report-format.md` — Audit Report Format (the `AUDIT:` mode deliverable) (~1274 tok)
- `business-continuity.md` — Business Continuity (BC) (~1612 tok)
- `caching.md` — Caching Strategy (~1767 tok)
- `compliance.md` — Security & Compliance Frameworks (NIST CSF 2.0 + SSDF, OWASP, SOC 2, Well-Architected) (~7076 tok)
- `containers-and-orchestration.md` — Docker & Kubernetes Standards (~5107 tok)
- `data-protection.md` — Data Protection & Privacy (GDPR / UK-GDPR / CCPA) (~4373 tok)
- `databases.md` — Database Standards (Postgres / Supabase, BigQuery, SQLite) (~6743 tok)
- `debugging.md` — Systematic Debugging (root cause, not symptom) (~2629 tok)
- `dev-environment-isolation.md` — Dev-Environment Isolation & Sandboxing (~3779 tok)
- `dev-environments.md` — IDE & Dev-Environment Standards (VS Code, Xcode, Google Antigravity) (~4328 tok)
- `diagrams-and-visual-docs.md` — Diagrams & Visual Documentation (~4013 tok)
- `disaster-recovery.md` — Disaster Recovery, Backups & Restore Drills (~3463 tok)
- `engineering-workflow.md` — Engineering Workflow (spec → plan → build → verify) (~4178 tok)
- `foss-adoption.md` — FOSS Adoption: The "Secure AND Tested" Gate (~2017 tok)
- `frontend-web-security.md` — Frontend / Web-App Security (~2237 tok)
- `gcp.md` — Google Cloud Platform Standards (~6279 tok)
- `github-actions.md` — GitHub Actions CI/CD Standards (~3514 tok)
- `github-teams.md` — GitHub Team Workflows (solo + agents -> human team) (~3750 tok)
- `google-apps-script.md` — Google Apps Script Discipline (~3609 tok)
- `iac-terraform.md` — Infrastructure as Code (Terraform on GCP) (~4826 tok)
- `javascript-and-typescript.md` — JavaScript & TypeScript Standards (~3533 tok)
- `local-and-agentic-ai-tools.md` — Local & Agentic AI Tooling (~2969 tok)
- `logging-and-monitoring.md` — Log Location, Rotation & Unattended-Job Monitoring (~2018 tok)
- `macos-app-bundles.md` — macOS App Bundle Standards (~1711 tok)
- `multi-agent-coordination.md` — Multi-Agent & Shared-Repo Coordination (~2016 tok)
- `my-environment.md` — My Environment Profile (~692 tok)
- `my-environment.template.md` — My Environment Profile (template) (~1054 tok)
- `observability-and-incident-response.md` — Observability & Incident Response (SRE) (~5596 tok)
- `package-managers.md` — Package Manager Standards (Homebrew, npm, mas) (~4138 tok)
- `python-typing-and-packaging.md` — Python Typing & Single-File→Package Layout (~816 tok)
- `python-web-apis.md` — Python Web API Standards (FastAPI / Uvicorn / psycopg) (~3288 tok)
- `resilience-engineering.md` — Resilience Engineering (designing software to degrade, not die) (~2727 tok)
- `scalability-and-system-design.md` — Scalability & System Design (the "-ilities") (~2086 tok)
- `secrets-and-key-rotation.md` — Secrets & Key Rotation Lifecycle (~2577 tok)
- `secure-data-processing.md` — Secure Data Processing for Commercial / Multi-Tenant Apps (~3425 tok)
- `standards-authoring.md` — Standards Authoring: distill sprawling conventions into checkable rules (~1441 tok)
- `testing-single-file.md` — Testing strategy for single-file scripts with module-level side effects (~427 tok)
- `testing.md` — Testing Strategy (strict, enforced gates) (~4701 tok)
- `threat-modeling-and-api-design.md` — Threat Modeling & API Design (~3548 tok)
- `ui-design-and-accessibility.md` — UI Design & Accessibility (~3308 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/senior-engineering-partner/scripts/

- `self-review.md` — Self-Review Checklist (verify before done) (~743 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/systematic-debugging/

- `condition-based-waiting.md` — Condition-Based Waiting (~875 tok)
- `CREATION-LOG.md` — Creation Log: Systematic Debugging Skill (~1059 tok)
- `defense-in-depth.md` — Defense-in-Depth Validation (~912 tok)
- `root-cause-tracing.md` — Root Cause Tracing (~1327 tok)
- `test-academic.md` — Academic Test: Systematic Debugging Skill (~164 tok)
- `test-pressure-1.md` — Pressure Test 1: Emergency Production Fix (~475 tok)
- `test-pressure-2.md` — Pressure Test 2: Sunk Cost + Exhaustion (~571 tok)
- `test-pressure-3.md` — Pressure Test 3: Authority + Social Pressure (~673 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/teach/

- `GLOSSARY-FORMAT.md` — GLOSSARY.md Format (~531 tok)
- `LEARNING-RECORD-FORMAT.md` — Learning Record Format (~690 tok)
- `MISSION-FORMAT.md` — MISSION.md Format (~386 tok)
- `RESOURCES-FORMAT.md` — RESOURCES.md Format (~481 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/test-driven-development/

- `testing-anti-patterns.md` — Testing Anti-Patterns (~2056 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/content/skills/writing-plans/

- `plan-document-reviewer-prompt.md` — Plan Document Reviewer Prompt Template (~428 tok)

## .claude/worktrees/architecture-diagram-skill/docs-site/scripts/

- `pull-skills.mjs` — Exports parseFrontmatter, isDenylisted, findNestedMarkdownFiles, buildSkillPage, syncSkills (~995 tok)
- `pull-skills.test.mjs` — content: makeTempDir (~2274 tok)

## .claude/worktrees/architecture-diagram-skill/docs/superpowers/plans/

- `2026-07-18-architecture-diagram-skill.md` — Architecture Diagram Skill Implementation Plan (~14044 tok)

## .claude/worktrees/architecture-diagram-skill/docs/superpowers/specs/

- `2026-07-14-issue-backlog-cycle-design.md` — Issue Backlog Cycle — Design (~2685 tok)
- `2026-07-14-token-optimization-design.md` — Token Optimization Pass — Design (~1979 tok)
- `2026-07-15-blume-docs-site-design.md` — Incorporate Blume: local harness docs site (~2334 tok)
- `2026-07-15-ponytail-bloat-tooling-design.md` — Bloat/debt tooling inspired by ponytail (~2804 tok)
- `2026-07-17-architecture-diagram-skill-design.md` — Architecture Diagram Generator skill (~3157 tok)

## .claude/worktrees/architecture-diagram-skill/hooks/

- `post-bash-truncate.js` — PostToolUse hook (Bash): warns when command output exceeds 15KB. (~294 tok)
- `post-compact-anatomy.sh` — PostCompact hook: re-injects OpenWolf anatomy after context compaction. (~483 tok)
- `post-skill-record.js` — PostToolUse hook (matcher: Skill): records each invoked skill as a (~364 tok)
- `post-skill-record.test.js` — SCRIPT: run, runRaw, withTmpCwd, markerPath (~1193 tok)
- `pre-compact-snapshot.sh` — PreCompact hook: stamps a compaction event in .wolf/cerebrum.md and reminds (~512 tok)
- `pre-read-check.js` — PreToolUse hook: guards context window before file reads. (~1251 tok)
- `pre-skill-gate.js` — PreToolUse hook (matcher: Skill): blocks invoking the writing-plans skill (~367 tok)
- `pre-skill-gate.test.js` — SCRIPT: run, runRaw, withTmpCwd, seedMarker (~1084 tok)
- `session-start.sh` — Unified SessionStart hook: Superpowers bootstrap + OpenWolf anatomy injection (~914 tok)
- `subagent-thin-harness.js` — SubagentStart hook: injects a short thin-harness digest into every (~520 tok)
- `subagent-thin-harness.md` — Declares with (~197 tok)
- `subagent-thin-harness.test.js` — SCRIPT: run, withTmpDigest (~775 tok)

## .claude/worktrees/architecture-diagram-skill/scripts/

- `wolf-debt-scan.js` — Deterministic scanner for wolf-debt: markers, used by the debt-ledger skill. (~957 tok)
- `wolf-debt-scan.test.js` — Split so this test file's own source never contains the literal, (~1367 tok)

## .claude/worktrees/architecture-diagram-skill/skills/architecture-diagram/

- `LICENSE` — Project license (~285 tok)
- `SKILL.md` — Architecture Diagram Skill (~2477 tok)

## .claude/worktrees/architecture-diagram-skill/skills/architecture-diagram/examples/

- `web-app-example.html` — Web App Architecture Diagram (~3488 tok)

## .claude/worktrees/architecture-diagram-skill/skills/architecture-diagram/resources/

- `template.html` — [PROJECT NAME] Architecture Diagram (~4976 tok)

## .claude/worktrees/architecture-diagram-skill/skills/ba-agent/.claude-plugin/

- `plugin.json` (~109 tok)

## .claude/worktrees/architecture-diagram-skill/skills/ba/

- `SKILL.md` — ba (~6233 tok)

## .claude/worktrees/architecture-diagram-skill/skills/bloat-audit/

- `SKILL.md` — Tags (~553 tok)

## .claude/worktrees/architecture-diagram-skill/skills/brand/

- `SKILL.md` — Brand Skill — Paul McKay Personal Brand (~1167 tok)

## .claude/worktrees/architecture-diagram-skill/skills/brand/.claude-plugin/

- `plugin.json` (~108 tok)

## .claude/worktrees/architecture-diagram-skill/skills/claude-infra-reference/

- `SKILL.md` — Claude Infra Reference (~1575 tok)

## .claude/worktrees/architecture-diagram-skill/skills/codebase-design/

- `DEEPENING.md` — Deepening (~638 tok)
- `DESIGN-IT-TWICE.md` — Design It Twice (~673 tok)
- `SKILL.md` — Codebase Design (~1520 tok)

## .claude/worktrees/architecture-diagram-skill/skills/dataviz/

- `SKILL.md` — DataViz Skill — Executive Data Visualization (~1996 tok)

## .claude/worktrees/architecture-diagram-skill/skills/debt-ledger/

- `SKILL.md` — Scan (~294 tok)

## .claude/worktrees/architecture-diagram-skill/skills/divergent-ideation/

- `SKILL.md` — Divergent Ideation (~729 tok)

## .claude/worktrees/architecture-diagram-skill/skills/domain-modeling/

- `ADR-FORMAT.md` — ADR Format (~686 tok)
- `CONTEXT-FORMAT.md` — CONTEXT.md Format (~572 tok)
- `SKILL.md` — Domain Modeling (~821 tok)

## .claude/worktrees/architecture-diagram-skill/skills/exec-dashboard/

- `SKILL.md` — Executive Dashboard Skill (~5489 tok)

## .claude/worktrees/architecture-diagram-skill/skills/github-issue-first/

- `SKILL.md` — GitHub Issue First (~1072 tok)

## .claude/worktrees/architecture-diagram-skill/skills/grill-me/

- `SKILL.md` (~37 tok)

## .claude/worktrees/architecture-diagram-skill/skills/grilling/

- `SKILL.md` (~167 tok)

## .claude/worktrees/architecture-diagram-skill/skills/handoff/

- `SKILL.md` (~262 tok)

## .claude/worktrees/architecture-diagram-skill/skills/html-export/

- `SKILL.md` — HTML Export Skill — Playwright Screenshot & PDF (~725 tok)

## .claude/worktrees/architecture-diagram-skill/skills/html-export/scripts/

- `html-export.js` — Usage: node html-export.js <input.html|url> [output-dir] [--wait <ms>] (~652 tok)

## .claude/worktrees/architecture-diagram-skill/skills/issue-backlog-cycle/

- `SKILL.md` — Issue Backlog Cycle (~1729 tok)

## .claude/worktrees/architecture-diagram-skill/skills/model-routing/

- `SKILL.md` — Model Routing (~515 tok)

## .claude/worktrees/architecture-diagram-skill/skills/session-reflect/

- `SKILL.md` — Session Reflect Skill (~834 tok)

## .claude/worktrees/architecture-diagram-skill/skills/teach/

- `GLOSSARY-FORMAT.md` — GLOSSARY.md Format (~531 tok)
- `LEARNING-RECORD-FORMAT.md` — Learning Record Format (~690 tok)
- `MISSION-FORMAT.md` — MISSION.md Format (~386 tok)
- `RESOURCES-FORMAT.md` — RESOURCES.md Format (~481 tok)
- `SKILL.md` — Teaching Workspace (~2441 tok)

## .claude/worktrees/architecture-diagram-skill/skills/wolf-init/

- `SKILL.md` — Wolf Init Skill (~504 tok)

## .claude/worktrees/architecture-diagram-skill/telemetry/

- `1p_failed_events.badaa411-a4e1-4cd7-a546-2de2859f0617.0bbe5eb5-1508-486a-beed-7e99ab30461f.json` (~103164 tok)

## .githooks/

- `pre-commit` — Local pre-commit lint/secret-scan gate — enforcement counterpart to (~455 tok)

## .superpowers/sdd/

- `.gitignore` — Git ignore rules (~1 tok)
- `progress.md` — SDD Progress Ledger — Token Optimization Pass (~878 tok)
- `review-083357b..66cf457.diff` — Review package: 083357b2abca8ba27d2adf2311eeaf01daf83b0f..HEAD (~245 tok)
- `review-3da0b9b..66cf457.diff` — Review package: 3da0b9b8a6ba960298d73cffa4beb892a0ed3686..HEAD (~8995 tok)
- `review-3da0b9b..dba8303.diff` — Review package: 3da0b9b8a6ba960298d73cffa4beb892a0ed3686..HEAD (~1827 tok)
- `review-bd77286..6564e28.diff` — Review package: bd77286b426cdbd1d0f78f51006b908b0e106ac9..HEAD (~3980 tok)
- `review-dba8303..bd77286.diff` — Review package: dba83039bbb6b562dba43c032702f1620826590a..HEAD (~681 tok)
- `task-1-brief.md` — Task 1: Create `claude-infra-reference` skill with moved content (~1907 tok)
- `task-1-report.md` — Task 1 Report: Create `claude-infra-reference` Skill (~308 tok)
- `task-2-brief.md` — Task 2: Create `model-routing` skill (~776 tok)
- `task-2-report.md` — Task 2 Report: Create `model-routing` skill (~263 tok)
- `task-3-brief.md` — Task 3: Trim CLAUDE.md — remove moved sections, condense Section 3 (+7), add pointers and model-routing bullet (~2979 tok)
- `task-3-report.md` — Task 3 Report: Trim CLAUDE.md (~928 tok)
- `task-5-brief.md` — Task 5: Add global permissions.deny (~1259 tok)
- `task-5-report.md` — Task 5 Report: Add global permissions.deny (~375 tok)

## .understand-anything/

- `.understandignore` — # **/*Test.java (~256 tok)
- `fingerprints.json` (~116279 tok)
- `knowledge-graph.json` (~127512 tok)
- `meta.json` (~45 tok)

## .understand-anything/.trash-1782444828/

- `assemble-review.json` — Declares prefixes (~540 tok)
- `assembled-graph.json` (~127299 tok)
- `batch-1.json` (~7711 tok)
- `batch-10.json` (~2411 tok)
- `batch-11.json` (~852 tok)
- `batch-12.json` — Declares readers (~2051 tok)
- `batch-13.json` (~2388 tok)
- `batch-14.json` (~943 tok)
- `batch-15.json` (~3832 tok)
- `batch-16.json` (~832 tok)
- `batch-17.json` (~4057 tok)
- `batch-18.json` (~671 tok)
- `batch-19.json` (~1470 tok)
- `batch-2.json` — Declares that (~8738 tok)
- `batch-20.json` (~876 tok)
- `batch-21.json` (~2662 tok)
- `batch-22.json` (~2198 tok)
- `batch-23.json` (~1214 tok)
- `batch-24.json` (~2046 tok)
- `batch-25.json` (~4833 tok)
- `batch-26.json` (~1618 tok)
- `batch-27.json` (~1172 tok)
- `batch-28.json` — Declares used (~3409 tok)
- `batch-29.json` — Declares so (~4978 tok)
- `batch-3.json` — Declares renames (~5595 tok)
- `batch-30.json` (~5038 tok)
- `batch-31.json` — Declares appears (~7995 tok)
- `batch-32.json` — Declares and (~7914 tok)
- `batch-4.json` (~7423 tok)
- `batch-5.json` — Declares spans (~5986 tok)
- `batch-6.json` (~718 tok)
- `batch-7.json` (~2490 tok)
- `batch-8.json` (~771 tok)
- `batch-9.json` (~1213 tok)
- `batches.json` (~27525 tok)
- `fingerprint-input.json` (~4292 tok)
- `layers.json` (~5860 tok)
- `review.json` (~1471 tok)
- `tour.json` — Declares where (~3674 tok)

## .understand-anything/.trash-1782444828/tmp/

- `arch-compact.json` (~54522 tok)
- `arch-input.json` (~61611 tok)
- `batch-dispatch-1.json` (~2004 tok)
- `batch-dispatch-10.json` (~613 tok)
- `batch-dispatch-11.json` (~233 tok)
- `batch-dispatch-12.json` (~381 tok)
- `batch-dispatch-13.json` (~576 tok)
- `batch-dispatch-14.json` (~198 tok)
- `batch-dispatch-15.json` (~1022 tok)
- `batch-dispatch-16.json` (~190 tok)
- `batch-dispatch-17.json` (~158 tok)
- `batch-dispatch-18.json` (~186 tok)
- `batch-dispatch-19.json` (~192 tok)
- `batch-dispatch-2.json` (~1800 tok)
- `batch-dispatch-20.json` (~206 tok)
- `batch-dispatch-21.json` (~598 tok)
- `batch-dispatch-22.json` (~389 tok)
- `batch-dispatch-23.json` (~251 tok)
- `batch-dispatch-24.json` (~245 tok)
- `batch-dispatch-25.json` (~474 tok)
- `batch-dispatch-26.json` (~622 tok)
- `batch-dispatch-27.json` (~315 tok)
- `batch-dispatch-28.json` (~316 tok)
- `batch-dispatch-29.json` (~1154 tok)
- `batch-dispatch-3.json` (~332 tok)
- `batch-dispatch-30.json` (~1222 tok)
- `batch-dispatch-31.json` (~1482 tok)
- `batch-dispatch-32.json` (~837 tok)
- `batch-dispatch-4.json` (~429 tok)
- `batch-dispatch-5.json` (~401 tok)
- `batch-dispatch-6.json` (~176 tok)
- `batch-dispatch-7.json` (~487 tok)
- `batch-dispatch-8.json` (~115 tok)
- `batch-dispatch-9.json` (~264 tok)
- `ua-assemble-final.cjs` — Declares fs (~226 tok)
- `ua-file-analyzer-input-1.json` (~518 tok)
- `ua-file-analyzer-input-12.json` (~309 tok)
- `ua-file-analyzer-input-16.json` (~157 tok)
- `ua-file-analyzer-input-20.json` (~155 tok)
- `ua-file-analyzer-input-3.json` (~246 tok)
- `ua-file-analyzer-input-8.json` (~108 tok)
- `ua-file-extract-results-1.json` (~8850 tok)
- `ua-file-extract-results-12.json` (~8025 tok)
- `ua-file-extract-results-16.json` (~495 tok)
- `ua-file-extract-results-20.json` (~892 tok)
- `ua-file-extract-results-3.json` (~36821 tok)
- `ua-file-extract-results-8.json` (~789 tok)
- `ua-import-map-input.json` (~10068 tok)
- `ua-import-map-output.json` (~5827 tok)
- `ua-inline-validate.cjs` — Declares fs (~833 tok)
- `ua-scan-files.json` (~12978 tok)
- `ua-tour-analyze.js` — Tour graph topology analyzer (~2115 tok)
- `ua-tour-input.json` (~99272 tok)
- `ua-tour-results.json` (~66205 tok)

## .understand-anything/intermediate/

- `scan-result.json` (~18834 tok)

## Agents/

- `.DS_Store` (~1640 tok)

## Agents/ba-agent/

- `.DS_Store` (~1640 tok)
- `CLAUDE.md` — BA Agent — Output Workspace (~544 tok)

## Agents/ba-agent/.claude/

- `settings.json` (~441 tok)

## Agents/ba-agent/.claude/rules/

- `openwolf.md` (~313 tok)

## Agents/ba-agent/outputs/

- `.DS_Store` (~1640 tok)
- `.gitkeep` (~0 tok)

## Agents/ba-agent/scripts/

- `validate-mermaid.sh` — Usage: bash scripts/validate-mermaid.sh outputs/process.mmd (~502 tok)

## Agents/ba-agent/skills/

- `assumption-log.md` — Skill: Assumption Log (~978 tok)
- `ba-package.md` — Skill: BA Package Assembly (~1670 tok)
- `comms-3p.md` — Skill: 3P Update Co-Authoring (~971 tok)
- `comms-faq.md` — Skill: FAQ Sheet Co-Authoring (~1294 tok)
- `data-mapping.md` — Skill: Data Mapping (~985 tok)
- `intake.md` — Skill: Brief Normaliser (~1672 tok)
- `planner.md` — Skill: Planner + Observe (~2038 tok)
- `png-render.md` — Skill: PNG Rendering + Docs Manifest (~1127 tok)
- `process-viz.md` — Skill: Process Visualisation (~968 tok)
- `raci-matrix.md` — Skill: RACI Matrix (~823 tok)
- `risk-register.md` — Skill: Risk Register (~1233 tok)
- `sop-writer.md` — Skill: SOP Writer (~1286 tok)
- `system-integration-viz.md` — Skill: System Integration Visualisation (~935 tok)
- `to-be-process.md` — Skill: To-Be Process Visualisation (~846 tok)
- `transformation-plan.md` — Skill: Transformation Plan (~1168 tok)
- `user-stories.md` — Skill: User Stories (~1053 tok)

## Agents/ba-agent/templates/

- `.DS_Store` (~1640 tok)
- `brief-template.md` — BA Brief Template (~407 tok)
- `context-object.json` (~84 tok)

## backups/

- `.claude.json.backup.1784487242529` — Declares breakdown (~12046 tok)
- `.claude.json.backup.1784493734798` (~12046 tok)
- `.claude.json.backup.1784502093340` (~12055 tok)
- `.claude.json.backup.1784510716066` — Declares breakdown (~12055 tok)
- `.claude.json.backup.1784517385442` — Declares breakdown (~12055 tok)

## brand/

- `brand-guide.md` — Paul McKay — Personal Brand Guide (~3689 tok)

## cache/

- `changelog.md` — Changelog (~100107 tok)

## chrome/

- `chrome-native-host` — Chrome native host wrapper script (~48 tok)

## docs-site/

- `.gitignore` — Git ignore rules (~4 tok)
- `blume.config.ts` (~68 tok)
- `package-lock.json` — npm lock file (~150416 tok)
- `package.json` — Node.js package manifest (~105 tok)
- `README.md` — Project documentation (~765 tok)

## docs-site/.blume/

- `astro.config.mjs` — Astro configuration (~2776 tok)
- `blume.manifest.json` (~19420 tok)
- `package.json` — Node.js package manifest (~48 tok)
- `tsconfig.json` — TypeScript configuration (~37 tok)

## docs-site/.blume/.astro/

- `content-assets.mjs` (~7 tok)
- `content-modules.mjs` (~56 tok)
- `content.d.ts` — Resolve an array of entry references from the same collection (~1698 tok)
- `fonts.d.ts` — Exports CssVariable (~47 tok)
- `settings.json` (~17 tok)
- `types.d.ts` — / <reference types="astro/client" /> (~32 tok)

## docs-site/.blume/.astro/fonts/google-c200761b0711fe56/6wRdeNJzEHNIsDAMAdKbdVLWIqu8b6-Bs-xVNZqplQw/google/

- `IBM Plex Mono-g1qQe502E2L5HJASCewr6ESluPLULMmp9EoHQnzwEVc-data.json` (~708 tok)
- `Inter Tight-zo3gvtb6YNVj6hcx8ORl1MiZ7yoBcrbKGyGGjzG_ptE-data.json` (~922 tok)
- `Inter-zo3gvtb6YNVj6hcx8ORl1MiZ7yoBcrbKGyGGjzG_ptE-data.json` (~934 tok)

## docs-site/.blume/src/

- `content.config.ts` — ","!**/.cache/**","!**/.git/**","!**/.next/**","!**/.turbo/**","!**/.vercel/**","!**/dist/**","!**/node_modules/**"], (~146 tok)
- `env.d.ts` — / <reference path="../.astro/types.d.ts" /> (~258 tok)

## docs-site/.blume/src/generated/

- `app.css` — /*.{astro,ts,tsx}"; (~9108 tok)
- `Ask.astro` — Ask AI is off (`ai.ask.enabled`), so the header's Ask trigger renders nothing. (~62 tok)
- `components.ts` — Exports mdxComponents, layoutOverrides (~71 tok)
- `data.json` (~26956 tok)
- `examples.css` — /*.{astro,jsx,svelte,ts,tsx,vue}"; (~1308 tok)
- `examples.ts` — Exports examplesBase, examples (~32 tok)
- `islands.ts` — Exports islandComponents (~21 tok)
- `openapi.json` (~1 tok)
- `raw-markdown.json` (~257036 tok)
- `search-client.ts` — Exports createSearch (~91 tok)
- `search.json` (~222213 tok)

## docs-site/.blume/src/pages/

- `[...slug].astro` — Astro: [...slug], 41 imports (~2613 tok)
- `[...slug].md.ts` — Exports prerender, getStaticPaths, GET (~174 tok)
- `[...slug].mdx.ts` — Exports prerender, getStaticPaths, GET (~171 tok)
- `404.astro` — Astro: 404 (~462 tok)
- `blume-search.json.ts` — Exports prerender, GET (~73 tok)

## docs-site/.blume/src/pages/og/

- `[...slug].png.ts` — Exports prerender, getStaticPaths, GET (~531 tok)

## docs-site/content/

- `index.mdx` — ~/.claude Reference (~286 tok)
- `openwolf.md` (~244 tok)

## docs-site/content/overview/

- `core-philosophy.md` (~131 tok)
- `infrastructure-layer.md` (~239 tok)
- `knowledge-graph.md` (~197 tok)
- `new-project-bootstrap.md` (~134 tok)
- `personal-knowledge-layer.md` (~202 tok)
- `process-layer.md` (~214 tok)
- `reference-pointers.md` (~121 tok)

## docs-site/content/skills/

- `ba.md` — ba (~6228 tok)
- `bloat-audit.md` — Tags (~413 tok)
- `brainstorming.md` — Brainstorming Ideas Into Designs (~3045 tok)
- `brand.md` — Brand Skill — Paul McKay Personal Brand (~1168 tok)
- `claude-infra-reference.md` — Claude Infra Reference (~1577 tok)
- `codebase-design.md` — Codebase Design (~1522 tok)
- `dataviz.md` — DataViz Skill — Executive Data Visualization (~1997 tok)
- `debt-ledger.md` — Scan (~202 tok)
- `divergent-ideation.md` — Divergent Ideation (~732 tok)
- `domain-modeling.md` — Domain Modeling (~822 tok)
- `exec-dashboard.md` — Executive Dashboard Skill (~5490 tok)
- `github-issue-first.md` — GitHub Issue First (~1073 tok)
- `grill-me.md` (~31 tok)
- `grilling.md` (~168 tok)
- `handoff.md` (~241 tok)
- `html-export.md` — HTML Export Skill — Playwright Screenshot & PDF (~725 tok)
- `issue-backlog-cycle.md` — Issue Backlog Cycle (~1729 tok)
- `model-routing.md` — Model Routing (~517 tok)
- `receiving-code-review.md` — Code Review Reception (~1588 tok)
- `requesting-code-review.md` — Requesting Code Review (~731 tok)
- `senior-engineering-partner.md` — ROLE AND CONTEXT (~21885 tok)
- `session-reflect.md` — Session Reflect Skill (~835 tok)
- `systematic-debugging.md` — Systematic Debugging (~2467 tok)
- `teach.md` — Teaching Workspace (~2421 tok)
- `test-driven-development.md` — Test-Driven Development (TDD) (~2473 tok)
- `using-git-worktrees.md` — Using Git Worktrees (~1868 tok)
- `verification-before-completion.md` — Verification Before Completion (~1039 tok)
- `wolf-init.md` — Wolf Init Skill (~505 tok)
- `writing-plans.md` — Writing Plans (~1815 tok)

## docs-site/content/skills/brainstorming/

- `spec-document-reviewer-prompt.md` — Spec Document Reviewer Prompt Template (~436 tok)
- `visual-companion.md` — Visual Companion Guide (~3305 tok)

## docs-site/content/skills/codebase-design/

- `DEEPENING.md` — Deepening (~638 tok)
- `DESIGN-IT-TWICE.md` — Design It Twice (~673 tok)

## docs-site/content/skills/domain-modeling/

- `ADR-FORMAT.md` — ADR Format (~686 tok)
- `CONTEXT-FORMAT.md` — CONTEXT.md Format (~572 tok)

## docs-site/content/skills/requesting-code-review/

- `code-reviewer.md` — Code Reviewer Prompt Template (~1301 tok)

## docs-site/content/skills/senior-engineering-partner/

- `README.md` — Project documentation (~4517 tok)

## docs-site/content/skills/senior-engineering-partner/.github/

- `pull_request_template.md` — What changed (~318 tok)

## docs-site/content/skills/senior-engineering-partner/evals/

- `README.md` — Project documentation (~828 tok)

## docs-site/content/skills/senior-engineering-partner/references/

- `audit-report-format.md` — Audit Report Format (the `AUDIT:` mode deliverable) (~1274 tok)
- `business-continuity.md` — Business Continuity (BC) (~1612 tok)
- `caching.md` — Caching Strategy (~1767 tok)
- `compliance.md` — Security & Compliance Frameworks (NIST CSF 2.0 + SSDF, OWASP, SOC 2, Well-Architected) (~7076 tok)
- `containers-and-orchestration.md` — Docker & Kubernetes Standards (~5107 tok)
- `data-protection.md` — Data Protection & Privacy (GDPR / UK-GDPR / CCPA) (~4373 tok)
- `databases.md` — Database Standards (Postgres / Supabase, BigQuery, SQLite) (~6743 tok)
- `debugging.md` — Systematic Debugging (root cause, not symptom) (~2629 tok)
- `dev-environment-isolation.md` — Dev-Environment Isolation & Sandboxing (~3779 tok)
- `dev-environments.md` — IDE & Dev-Environment Standards (VS Code, Xcode, Google Antigravity) (~4328 tok)
- `diagrams-and-visual-docs.md` — Diagrams & Visual Documentation (~4013 tok)
- `disaster-recovery.md` — Disaster Recovery, Backups & Restore Drills (~3463 tok)
- `engineering-workflow.md` — Engineering Workflow (spec → plan → build → verify) (~4178 tok)
- `foss-adoption.md` — FOSS Adoption: The "Secure AND Tested" Gate (~2017 tok)
- `frontend-web-security.md` — Frontend / Web-App Security (~2237 tok)
- `gcp.md` — Google Cloud Platform Standards (~6279 tok)
- `github-actions.md` — GitHub Actions CI/CD Standards (~3514 tok)
- `github-teams.md` — GitHub Team Workflows (solo + agents -> human team) (~3750 tok)
- `google-apps-script.md` — Google Apps Script Discipline (~3609 tok)
- `iac-terraform.md` — Infrastructure as Code (Terraform on GCP) (~4826 tok)
- `javascript-and-typescript.md` — JavaScript & TypeScript Standards (~3533 tok)
- `local-and-agentic-ai-tools.md` — Local & Agentic AI Tooling (~2969 tok)
- `logging-and-monitoring.md` — Log Location, Rotation & Unattended-Job Monitoring (~2018 tok)
- `macos-app-bundles.md` — macOS App Bundle Standards (~1711 tok)
- `multi-agent-coordination.md` — Multi-Agent & Shared-Repo Coordination (~2016 tok)
- `my-environment.md` — My Environment Profile (~692 tok)
- `my-environment.template.md` — My Environment Profile (template) (~1054 tok)
- `observability-and-incident-response.md` — Observability & Incident Response (SRE) (~5596 tok)
- `package-managers.md` — Package Manager Standards (Homebrew, npm, mas) (~4138 tok)
- `python-typing-and-packaging.md` — Python Typing & Single-File→Package Layout (~816 tok)
- `python-web-apis.md` — Python Web API Standards (FastAPI / Uvicorn / psycopg) (~3288 tok)
- `resilience-engineering.md` — Resilience Engineering (designing software to degrade, not die) (~2727 tok)
- `scalability-and-system-design.md` — Scalability & System Design (the "-ilities") (~2086 tok)
- `secrets-and-key-rotation.md` — Secrets & Key Rotation Lifecycle (~2577 tok)
- `secure-data-processing.md` — Secure Data Processing for Commercial / Multi-Tenant Apps (~3425 tok)
- `standards-authoring.md` — Standards Authoring: distill sprawling conventions into checkable rules (~1441 tok)
- `testing-single-file.md` — Testing strategy for single-file scripts with module-level side effects (~427 tok)
- `testing.md` — Testing Strategy (strict, enforced gates) (~4701 tok)
- `threat-modeling-and-api-design.md` — Threat Modeling & API Design (~3548 tok)
- `ui-design-and-accessibility.md` — UI Design & Accessibility (~3308 tok)

## docs-site/content/skills/senior-engineering-partner/scripts/

- `self-review.md` — Self-Review Checklist (verify before done) (~743 tok)

## docs-site/content/skills/systematic-debugging/

- `condition-based-waiting.md` — Condition-Based Waiting (~875 tok)
- `CREATION-LOG.md` — Creation Log: Systematic Debugging Skill (~1059 tok)
- `defense-in-depth.md` — Defense-in-Depth Validation (~912 tok)
- `root-cause-tracing.md` — Root Cause Tracing (~1327 tok)
- `test-academic.md` — Academic Test: Systematic Debugging Skill (~164 tok)
- `test-pressure-1.md` — Pressure Test 1: Emergency Production Fix (~475 tok)
- `test-pressure-2.md` — Pressure Test 2: Sunk Cost + Exhaustion (~571 tok)
- `test-pressure-3.md` — Pressure Test 3: Authority + Social Pressure (~673 tok)

## docs/superpowers/plans/

- `2026-07-14-claude-md-token-optimization.md` — CLAUDE.md Token Optimization (Phase 1: ~/.claude) Implementation Plan (~8421 tok)
- `2026-07-14-issue-backlog-cycle.md` — Issue Backlog Cycle Implementation Plan (~5622 tok)
- `2026-07-14-nhl-stats-claudeignore.md` — NHL Stats Project Token Optimization (Phase 2) Implementation Plan (~1770 tok)
- `2026-07-15-blume-docs-site.md` — Blume Harness Docs Site Implementation Plan (~8528 tok)
- `2026-07-15-ponytail-bloat-tooling.md` — Ponytail-Inspired Bloat/Debt Tooling Implementation Plan (~7398 tok)
- `2026-07-18-architecture-diagram-skill.md` — Architecture Diagram Skill Implementation Plan (~14044 tok)

## docs/superpowers/specs/

- `2026-07-14-issue-backlog-cycle-design.md` — Issue Backlog Cycle — Design (~2685 tok)
- `2026-07-14-token-optimization-design.md` — Token Optimization Pass — Design (~1979 tok)
- `2026-07-15-blume-docs-site-design.md` — Incorporate Blume: local harness docs site (~2334 tok)
- `2026-07-15-ponytail-bloat-tooling-design.md` — Bloat/debt tooling inspired by ponytail (~2804 tok)
- `2026-07-17-architecture-diagram-skill-design.md` — Architecture Diagram Generator skill (~3195 tok)
