...
## Browser Automation
- A `playwright` MCP server (`@playwright/mcp`) is configured in `mcp_servers.json` for interactive browser automation (navigate/click/type/screenshot/DOM read) — available after a session restart. Distinct from the `html-export` skill (static screenshot/PDF export of a finished page); use `playwright` when you need to actually interact with a running app. Note: `html-export`'s own Playwright pipeline is currently broken (ESM/CJS conflict in `~/.claude/package.json` + no local `playwright` npm package) — needs a fix before it works again.
