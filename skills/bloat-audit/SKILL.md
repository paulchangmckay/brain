...
name: bloat-audit
description: >
  Whole-repo audit for over-engineering: dead code, single-implementation
  abstractions, hand-rolled reimplementations of the stdlib or an installed
  dependency, unused flexibility. Ranked, read-only, one-shot report — does
  not apply fixes. Use when the user says "audit this codebase for
  over-engineering", "what can I delete from this repo", "find bloat",
  "bloat audit", or "audit for bloat". Correctness bugs, security holes, and
  performance are out of scope — route those to senior-engineering-partner's
  AUDIT: mode or a normal code review instead.
---
